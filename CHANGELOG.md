...
### Fixed
- **Dark-mode polish across Audible Accounts and Downloads.** Cards and outlines rendered bright white, several labels were dark text on a dark background (hard to read), and some controls didn't match the rest of the app. The cards, filter pills, Scan Library button, toggles and text now match the Settings page; the "This device" badge is gold and clearly visible; and an account that needs re-authentication no longer shows a green "authenticated" check.
- **"This device" badge now works in every browser.** It was cookie-dependent: the server identified the caller's session by hashing the `refresh_token` cookie the browser sent with the session-list request, and some browsers (a user's main Brave profile) never attached that cookie, so the badge never appeared there even though login and refresh worked. Login and refresh now return the caller's own session id, and the client marks its own row by id (falling back to the server's cookie-hash flag), so the badge no longer depends on that request carrying the cookie. The app HTML (`index.html`) is now served `no-cache` so an update is never masked by a stale cached page still pointing at an old bundle.
