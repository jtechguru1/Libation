...
- **Frontend**: React 18 + Vite + Tailwind CSS, built to `/app/static` and served as static files by FastAPI
  ...
  - `index.html` is served with `Cache-Control: no-cache, must-revalidate` (both the fallback and the direct-file branch when the file *is* index.html), so the SPA shell is always revalidated and points at the current hashed bundle — a stale cached page can't mask an update. The content-hashed files under `/assets` keep default caching.
...
- `GET /api/auth/sessions` / `DELETE /api/auth/sessions/{id}` / `DELETE /api/auth/sessions` — session management for the logged-in user. The list hashes the request's `refresh_token` cookie and sets `is_current: bool` on the matching row (missing cookie → all false), then sorts the caller's own session first. **The "This device" badge no longer depends on that cookie**: `login`/`verify-2fa`/`refresh` return the caller's own `session_id` (see below), the client stores it in `AuthContext.currentSessionId`, and the UI marks a row as the current device when `row.id === currentSessionId || row.is_current` — the id match wins, the server cookie-hash flag is the fallback. That combined boolean drives both the badge and hiding the per-row revoke button. Fixes the badge going missing in browsers that withhold the cookie from this request (e.g. hardened Brave profiles).
- **`session_id` in `TokenResponse`** — `create_session` returns `(raw_token, session.id)`; `login`, `verify-2fa`, and `refresh` pass that id back to the client as `session_id`. This is what lets the client mark its own "This device" row by id instead of relying on the server hashing the request cookie.
